...
class PIIMasker:
    """Class to handle masking anclass PIIMasker:
    """Class to handle masking and unmasking of PII data in emails."""
    ...
    def unmask_text(self, masked_text: str) -> str:
        ...
        for entity_id, entity in self.entity_store.items():
            masked_text = masked_text.replace(f"[{entity['type']}]", entity['value'], 1)
        return masked_text
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df_processedd unmasking of PII data in emails."""
